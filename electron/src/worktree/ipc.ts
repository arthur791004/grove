...
import { addWorktree, deleteBranch, headSha, listGroveBranches, removeWorktree, resolveRepoRoot, worktreeStatus, type WorktreeStatus } from './git';
...
export function registerWorktreeHandlers() {
  ipcMain.handle('workspace:fork', async (_e, req: ForkRequest): Promise<ForkResult> => {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      if (r.ok) {
        registry.add({ workspaceId: req.workspaceId, repoRoot, branch, worktreePath, createdAt: Date.now() });
  ...
  ipcMain.handle('workspace:close', async (_e, req: CloseRequest): Promise<{ removed: boolean; branchDeleted: boolean }> => {
    ...
    if (!r.ok) {
      ...
      throw new Error(`git worktree remove failed: ${r.stderr.trim() || `git exited ${r.status}`}`);
    }
    registry.remove(req.workspaceId);
  ...
  ipcMain.handle('workspace:status', async (_e, req: StatusRequest): Promise<WorktreeStatus | null> => {
  ...
  ipcMain.handle('workspace:list-grove-branches', async (_e, req: { liveWorkspaceIds?: string[]; cwds?: string[] }): Promise<OrphanBranch[]> => {
  ...
  ipcMain.handle('workspace:delete-branches', async (_e, req: { entries: OrphanBranch[] }): Promise<{ deleted: number; errors: Array<{ branch: string; message: string }> }> => {
